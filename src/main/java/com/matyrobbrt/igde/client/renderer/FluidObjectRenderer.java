...
// TODO allow rendering fluids using their buckets
public class FluidObjectRenderer implements ObjectRenderer<Fluid> {
    ...
    private void drawFluid(PoseStack poseStack, final int width, final int height, FluidStack fluidStack) {
        ...
        int scaledAmount = (amount * height) / 1000;
    ...
    @Override
    public void render(Fluid object, Screen screen, PoseStack poseStack, int x, int y) {
        if (object != null) {
            poseStack.pushPose();
            {
                poseStack.translate(x, y, 0);
                RenderSystem.enableBlend();
                drawFluid(poseStack, 16, 16, new FluidStack(object, 1000));
